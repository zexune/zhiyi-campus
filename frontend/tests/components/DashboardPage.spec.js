...
beforeEach(() => {
  getSchools.mockResolvedValue({ data: [{ id: 1, name: '上海大学' }] })
  getDashboard.mockResolvedValue({ data: dashboard() })
  getTradeHeatmap.mockResolvedValue({ data: [{ location: '图书馆', count: 2 }] })
})
...
test('后台大盘同时呈现统计、趋势、热力和最近治理记录', async () => {
  ...
  assert.deepEqual(getSchools.mock.calls[0], [{ status: 'ACTIVE' }])
  assert.deepEqual(getDashboard.mock.calls[0], [null])
  assert.deepEqual(getTradeHeatmap.mock.calls[0], [null])
})
...
test('切换学校后用同一 schoolId 重新查询大盘和热力图', async () => {
  ...
  assert.deepEqual(getDashboard.mock.calls.at(-1), [1])
  assert.deepEqual(getTradeHeatmap.mock.calls.at(-1), [1])
...
test('大盘失败展示独立重试入口，不被学校和热力图失败吞掉', async () => {
  ...
  getSchools.mockRejectedValueOnce(new Error('schools unavailable'))
  getTradeHeatmap.mockRejectedValueOnce(new Error('heatmap unavailable'))
  const wrapper = mount(DashboardPage, { global })
  await flushPromises()
