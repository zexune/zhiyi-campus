...
beforeEach(() => {
  vi.mocked(adminLogin).mockResolvedValue({ code: 200, message: 'ok', data: { token: 'jwt', user: adminUser } })
})
...
test('空表单提交被声明式校验拦截，展示行内错误且不请求接口', async () => {
  ...
  assert.equal(vi.mocked(adminLogin).mock.calls.length, 0)
})
...
test('填写后提交通过校验，以表单值调用 adminLogin 并跳转管理后台', async () => {
  ...
  assert.deepEqual(vi.mocked(adminLogin).mock.calls[0], [{ username: 'admin', password: '123456' }])
})
