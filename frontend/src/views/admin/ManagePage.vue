      ...
      <div class="tool-grid">
        ...
        <div class="tool-card card">
          ...
          <p class="tool-card__desc muted">
            搜索商品后选择目标并执行运营下架。该操作只改变商品状态，不自动处罚或封禁卖家。
          </p>
        ...
        <div class="tool-card card user-ban-card">
          ...
          <div v-if="banForm.users.length" class="user-list">
            <div
              v-for="user in banForm.users"
              :key="user.id"
              class="user-item card card--flat"
              :class="{ active: banForm.selectedId === user.id }"
          ...
          <template v-if="banForm.selected">
            <div class="preview-card card card--flat">
              <div class="preview-row"><span class="muted">用户：</span><strong>{{ banForm.selected.nickname }}（{{ banForm.selected.studentId }}）</strong></div>
              <div class="preview-row"><span class="muted">账号状态：</span>{{ userStatusLabel(banForm.selected.status) }}</div>
        ...
        <div class="tool-card card topic-card">
          ...
          <div class="field"><label>商品标签（可选）</label><input v-model.trim="topicForm.filterTag" class="input" maxlength="50" placeholder="如：毕业季"></div>
...
async function handleForceOffShelf() {
  ...
  try {
    await ElMessageBox.confirm(
      `确认强制下架「${it.title}」(#${it.id})？此操作只改变商品状态，不会自动扣分或封禁卖家。`,
      '强制下架',
...
async function searchBanUsers() {
  ...
  } finally { banForm.searching = false }
}
...
async function handleBanUser() {
  ...
  } finally { banForm.submitting = false }
}
