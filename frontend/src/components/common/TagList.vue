...
<style scoped>
.tag-list { min-width: 0; display: flex; align-items: center; flex-wrap: wrap; gap: 5px; }
.tag-list__item {
  max-width: 100%; min-width: 0; padding: 2px 8px; overflow-wrap: anywhere;
  border: 1.5px dashed var(--ink-soft); border-radius: 999px;
  background: var(--paper-deep); color: var(--ink-soft); font-size: 11px;
  font-weight: 700; line-height: 1.45; text-align: left; white-space: normal;
  cursor: pointer; transition: background-color .15s, border-color .15s, color .15s;
}
.tag-list__item:hover, .tag-list__item:focus-visible {
  outline: none; border-style: solid; border-color: var(--ink);
  background: var(--yellow); color: var(--ink);
}
.tag-list--compact { gap: 4px; margin-top: 4px; }
.tag-list--compact .tag-list__item { padding: 1px 7px; font-size: 10.5px; }
</style>
