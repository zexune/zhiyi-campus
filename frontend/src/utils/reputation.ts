...
/** 六个维度的固定顺序与中文标签（与后端字段一一对应；键值联合由本数组派生） */
...
/** ReputationVO 的形状（六维分值；接口侧全部字段可选，缺失按 0 处理） */
export type ReputationVo = Partial<Record<ReputationKey, number | string | null | undefined>>
...
export function overallScore(vo: ReputationVo | null | undefined): number {
...
/** 综合分 → 评级文案 */
export function reputationGrade(score: number): '信誉极佳' | '信誉良好' | '信誉一般' | '信誉待提升' {
