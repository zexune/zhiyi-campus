...
- **本地内容治理**：本地规则经 Aho–Corasick 自动机单趟匹配违规关键词（含全拉丁拼音拼写变体 daixie≈代写），命中后隐藏商品并交由管理员复核；支持用户举报、卖家整改和限时申诉。
- **收藏与站内沟通**：商品收藏、买卖双方会话、未读消息统计和管理员客服会话。
- **交易闭环**：平台钱包、充值流水、创建/取消订单、确认收货与交易评价；资金操作必须携带 `X-Idempotency-Key` 幂等键，同键重复请求恰好执行一次并重放同一结果。
- **信誉与成长体系**：目录化经验事件（完成订单/发布商品/收到好评/首次完善资料，含每日上限、一次性任务与同买家-卖家对回购衰减）、10 级成长曲线、六维信誉雷达（180 天时间窗口、最小样本门槛、首响 EWMA、处罚随时间线性衰减，每维携带样本量与归因文案）、校园关系标签和违规处罚记录。
...
- **认证事务边界与准入背压**：登录/密保/注册不以方法级事务包裹——BCrypt 校验与哈希（单次几十毫秒纯 CPU）在无数据库连接状态下执行，仅多语句原子小节（临时封禁恢复、改密+版本推进）用短事务收拢；`AuthAdmissionGate` 在触碰任何数据库之前限制并发认证数（`zhiyi.auth.admission-slots`，默认 0 = 按 2×CPU 核数自动推导，可显式覆盖），超限快速返回 `AUTH_BUSY`（429 + Retry-After）。否则 CPU 饱和会级联耗尽连接池并拖垮全部依赖数据库的接口。
